...
export default function AppShell() {
  ...
  const hasAdmin = user?.roles?.some(r => (r.code || r) === 'ADMIN' || (r.code || r) === 'system_admin');
    ...
    <div className="flex h-screen flex-col bg-[var(--color-background)] text-[var(--color-foreground)]">
      ...
      <header className="flex h-12 shrink-0 items-center gap-4 border-b border-[var(--color-border)] bg-[var(--color-card)] px-4">
        <NavLink to="/" className="flex items-center gap-2 font-semibold tracking-tight">
          <span className="grid h-6 w-6 place-items-center rounded bg-[var(--color-primary)] text-[11px] font-bold text-[var(--color-primary-foreground)]">IV</span>
        ...
        <div className="flex flex-1 justify-center">
          {statusLabel && (
            <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-medium ${toneClasses[statusTone || 'neutral']}`}>
          ...
          {showNotifications && (
            <div className="absolute right-0 top-8 z-50 w-80 rounded-lg border border-[var(--color-border)] bg-white shadow-lg">
              <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-2">
                ...
                <button onClick={() => setShowNotifications(false)} className="text-xs text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)]">✕</button>
              </div>
              <div className="max-h-64 overflow-y-auto">
                {notifications.length === 0 ? (
                  <p className="px-4 py-6 text-center text-xs text-[var(--color-muted-foreground)]">No unread notifications</p>
                ) : (
                  notifications.map(n => (
                    <div key={n.id} className="border-b border-[var(--color-border)] px-4 py-3 text-xs last:border-0">
                      <p className="font-medium text-[var(--color-foreground)]">{n.title || n.message}</p>
                      {n.title && n.message && <p className="mt-0.5 text-[var(--color-muted-foreground)]">{n.message}</p>}
                    </div>
                  ))
                )}
              </div>
